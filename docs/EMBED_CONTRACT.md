...
Under the hood `embed_client` selects between two backends (= remote
HTTP `/embed`, set via `BGE_EMBED_URL`; or in-process BGE-M3) and
asserts `embed_norm.assert_normalized` / `assert_batch_normalized` on
...
- helper: [`embed_client.py`](../embed_client.py)
- assertion: [`embed_norm.py`](../embed_norm.py)
