...
Migration `030` 以降は共有海馬で「どの器の経験か」を失わないよう、日記 writer の
`writer_host` / `writer_runtime` / `memory_mode` と、材料になった各 session の
`session_id` / `runtime` / `model` / `host` を `source_provenance` (JSONB) に保存します。
会話側の `source_host` は ingest 時の hostname、またはコンテナ用の明示ラベル
`HIPPOCAMPUS_SOURCE_HOST` です。writer 側は `HIPPOCAMPUS_WRITER_HOST` と
`HIPPOCAMPUS_WRITER_RUNTIME` で上書きできます。
