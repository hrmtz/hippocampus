...
The write path above (`propose`/`apply`) mutates `body_md`; the **read** path
renders those bodies as HTML over HTTP — read-only (no DB mutation, PG via sops).

```bash
# localhost-only by default (port 8087); public access is ONLY via the tunnel
hippocampus wiki serve                            # = --host 127.0.0.1 --port 8087
WIKI_SERVE_HOST=0.0.0.0 hippocampus wiki serve    # expose on the WSL host / tailnet (opt-in)
```

- **systemd**: `hippocampus-wiki.service` runs `run_wiki_server.sh` with
  `WIKI_SERVE_HOST=127.0.0.1` (in the base unit **and** a `bind-localhost.conf`
  drop-in) — the server **never binds a public interface itself**. Restart to
  reload code + re-prewarm the render cache:
  `systemctl --user restart hippocampus-wiki.service`.
- **Public exposure** is a dedicated cloudflared tunnel
  (`cloudflared-hippo-wiki.service`, config `~/.cloudflared/hippo-wiki-config.yml`)
  dialing `127.0.0.1:8087` → **your-notes-host.example.com** (unlisted).
- **Render**: pandoc (GFM tables + `--toc` + MathML) → HTML, plus render-time
  transforms — `[[wikilink]]` / `[[slug#anchor|alias]]` resolution and TeX-style
  autonumber (opt-in per page via the `<!-- wiki:autonumber -->` sentinel).
  Degrades to an escaped `<pre>` surface if pandoc is absent. `body_md` in the DB
  is never touched — every transform is render-time only.
- **Nav** (v1.13.0): the **header** carries only the `◆ index` link back to the
  portal; the **footer** carries the *reading path* — the current page's
  **same-category** siblings with prev/next buttons. Other categories are omitted
  (they live in the portal index at `/`). Curated group order = reading order;
  last-in-group shows no next arrow. (`_nav_header_html` / `_nav_footer_html` in
  `wiki.py`.)
- **Cache**: rendered pages are TTL-cached (~60 s) and served with
  `Cache-Control` + `ETag`. A `body_md` edit (`apply`) surfaces after the TTL; a
  **code** change (render/nav) needs a service restart, which re-prewarms.
