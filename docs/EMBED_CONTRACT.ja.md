...
内部では `embed_client` が 2 つのバックエンド（= `BGE_EMBED_URL` で設定する
リモート HTTP `/embed`、または in-process BGE-M3）から選択し、すべての return
パスで `embed_norm.assert_normalized` / `assert_batch_normalized` をアサート
します。ヘルパは次を行います。
