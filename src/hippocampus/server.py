            ...
            cur.execute("""
                WITH dense_ranked AS (
                    SELECT
                        ...
                        ROW_NUMBER() OVER (ORDER BY m.dense <#> %s::halfvec) AS rnk
                ...
                trgm_ranked AS (
                    SELECT
                        m.id,
                        ROW_NUMBER() OVER (
                            ORDER BY word_similarity(%s, m.content) DESC
                        ) AS rnk
                    FROM personal.messages m
                    WHERE m.content ~* %s
                    ORDER BY word_similarity(%s, m.content) DESC
                ...
                top_ids AS (
                    SELECT id, rrf_score FROM scored ORDER BY rrf_score DESC LIMIT %s
                )
                SELECT
                    m.conv_id, m.role, m.content, m.ts,
                    c.title, c.platform,
...
def search_conversations(query: str, top_k: int = 10, platform: str | None = None) -> str:
    ...
    with _db_cursor(cursor_factory=psycopg2.extras.RealDictCursor) as (conn, cur):
        ...
        """, {"vec": vec, "platform": platform, "top_k": top_k, "candidate_k": candidate_k})
        rows = cur.fetchall()
...
def get_conversation(conv_id: str) -> str:
    ...
    with _db_cursor(cursor_factory=psycopg2.extras.RealDictCursor) as (conn, cur):
        ...
        if _CAPS.get("personal", True):
            cur.execute("SELECT conv_id FROM personal.conversations WHERE conv_id = %s", (conv_id,))
            if cur.fetchone():
                schema = "personal"
        if schema is None and _CAPS.get("library", True):
            cur.execute("SELECT conv_id FROM library.conversations WHERE conv_id = %s", (conv_id,))
            if cur.fetchone():
                schema = "library"
        if schema is None:
        ...
        cur.execute(f"SELECT title, platform, started_at FROM {schema}.conversations WHERE conv_id = %s", (conv_id,))
        meta = cur.fetchone()
...
def _recency_query(days: int, limit: int, platform: str | None, project: str | None):
    ...
    if project:
        where_parts.append("c.title ILIKE %s")
    ...
    sql = f"""
        ...
        HAVING COALESCE(c.ended_at, MAX(m.ts), c.started_at) >= NOW() - INTERVAL '1 day' * %s
...
def search_facts(query: str, top_k: int = 10) -> str:
    ...
    with _db_cursor(cursor_factory=psycopg2.extras.RealDictCursor) as (conn, cur):
        cur.execute("""
            ...
            fts_ranked AS (
                SELECT
                    f.id,
                    ROW_NUMBER() OVER (
                        ORDER BY ts_rank_cd(f.fts, plainto_tsquery('simple', %s)) DESC
                    ) AS rnk
                FROM personal.extracted_facts f
                WHERE f.fts @@ plainto_tsquery('simple', %s)
                ORDER BY ts_rank_cd(f.fts, plainto_tsquery('simple', %s)) DESC
                LIMIT %s
            ),
            all_ids AS (
        ...
        """, (
            vec, vec, candidate_k,
            fts_query, fts_query, fts_query, candidate_k,
            top_k,
        ))
        rows = cur.fetchall()
...
_TOOL_NEEDS_EMBED = {"search_personal_memory", "search_conversations", "search_library", "search_facts"}
...
_CAPS: dict[str, bool] = {"personal": True, "library": True, "ghost": True, "personal_facts": True}
...
def _probe_capabilities() -> dict[str, bool]:
    ...
    *structurally* absent schema, not on a transient hiccup — flapping the tool
    ...
    """
    caps = {"personal": True, "library": True, "ghost": True, "personal_facts": True}
    try:
        ...
        return {"personal": True, "library": True, "ghost": True, "personal_facts": True}
...
def _gate_tools() -> None:
    ...
    if not _SETTINGS.embed_configured:
        print("[hippocampus] no embed backend configured (BGE_EMBED_URL unset, "
              "EMBED_PROVIDER != bge-inprocess); semantic search tools disabled",
