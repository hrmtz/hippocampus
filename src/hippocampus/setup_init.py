...
EMBED_CHOICES = ("bge-ondemand", "bge-http", "bge-inprocess", "none")
DEFAULT_BGE_ONDEMAND_IDLE_SECONDS = 300
...
def _gather_embed(args: argparse.Namespace) -> tuple[str, str, str, int | None]:
    """Return (choice, bge_url, bge_token, ondemand_idle)."""
    choice = args.embed
    if not choice:
        _require_tty("embed provider choice")
        print("Semantic embedding backend — choose explicitly (no default):")
        print("  1) bge-ondemand   local compose BGE-M3; starts on first semantic use")
        print("                    and exits after an idle timeout")
        print("  2) bge-http       remote or manually managed BGE-M3 HTTP server")
        print("  3) bge-inprocess  local in-process model (~6GB download/RAM,")
        print("                    requires `pip install hippocampus-mcp[bge-local]`)")
        print("  4) none           semantic tools stay off (keyword search only)")
        mapping = {"1": "bge-ondemand", "2": "bge-http",
                   "3": "bge-inprocess", "4": "none"}
        while True:
            raw = input("embed backend [1/2/3/4]: ").strip().lower()
            choice = mapping.get(raw, raw if raw in EMBED_CHOICES else "")
            if choice:
                break
            print("please answer 1, 2, 3 or 4 "
                  "(or bge-ondemand / bge-http / bge-inprocess / none)")
    ...
    elif choice == "bge-ondemand":
        from .embed.ondemand import read_or_create_token  # noqa: PLC0415

        if args.bge_token_env:
            bge_token = os.environ.get(args.bge_token_env, "")
        if not bge_token:
            bge_token = read_or_create_token()
        ondemand_idle = args.bge_ondemand_idle_seconds
        if ondemand_idle < 30:
            raise InitError("--bge-ondemand-idle-seconds must be >= 30")
    return choice, bge_url.rstrip("/"), bge_token, ondemand_idle
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--bge-ondemand-idle-seconds", type=int,
                        default=DEFAULT_BGE_ONDEMAND_IDLE_SECONDS,
                        help="idle seconds before bge-ondemand exits "
                             f"(default {DEFAULT_BGE_ONDEMAND_IDLE_SECONDS})")
...
def _run(args: argparse.Namespace) -> int:
    ...
    embed_choice, bge_url, bge_token, ondemand_idle = _gather_embed(args)
    ...
    elif embed_choice == "bge-ondemand":
        values["EMBED_PROVIDER"] = "bge-ondemand"
        values["BGE_EMBED_TOKEN"] = bge_token
        values["BGE_ONDEMAND_IDLE_SECONDS"] = str(
            ondemand_idle or DEFAULT_BGE_ONDEMAND_IDLE_SECONDS)
        values["BGE_RESTART_POLICY"] = "no"
        values["HIPPOCAMPUS_COMPOSE_DIR"] = os.getcwd()
