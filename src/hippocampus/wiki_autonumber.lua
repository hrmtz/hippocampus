...
-- Exact opt-in sentinel. The filter deletes ONLY a RawBlock/RawInline whose
-- trimmed text equals this comment verbatim (design rhythm-workshop §7.2 / I6).
-- A substring match (the old `:find("wiki:autonumber")`) erased any <script>
-- that merely *mentioned* the string — a script-heavy page's engine could
-- vanish at render. A raw <script>...</script> becomes ONE RawBlock whose text
-- is the whole element, so an exact-match never touches it.
local SENTINEL = "<!-- wiki:autonumber -->"

local function is_sentinel(text)
  -- trim leading/trailing whitespace, then require an exact match
  return (text:gsub("^%s+", ""):gsub("%s+$", "")) == SENTINEL
end
...
function Pandoc(doc)
  ...
  doc.blocks = doc.blocks:walk({
    RawBlock = function(rb)
      if rb.format == "html" and is_sentinel(rb.text) then
        return {}
      end
    end,
    RawInline = function(ri)
      ...
      if ri.format == "html" and is_sentinel(ri.text) then
        return {}
      end
    end,
    Header = function(h)
