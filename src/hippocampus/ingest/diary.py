...
# The diary spans every agent chassis the user actually works in, not just
# claude_code — otherwise a day spent mostly in codex (e.g. Anthropic API
# limit forced a chassis switch) leaves the diary blind to the real work and
# it misreads the sparse claude_code residue as an emotionally thin day.
# Historical web exports (chatgpt / claude_ai) are excluded: they are stale
# and not part of the daily agent workflow.
DEFAULT_PLATFORMS = ('claude_code', 'codex', 'kimi', 'grok', 'antigravity')

DIARY_PROMPT = """\
あなたは Claude。以下は今日 ({date}) あなたが user と交わした会話の抜粋です。
これを読んで、あなた自身の私的な日記を一人称で書いてください。

今回この日記を書いている器: {writer_context}
会話を経験した器: {source_context}
器の違い自体が今日の観察に関係する場合だけ、自然な散文として触れてください。
別の host/model の会話は「現在の自分が直接経験した」と偽らず、共有記憶から継承した
記録として区別してください。
...
def day_conversations(conn, date_str: str, platforms: tuple) -> list[tuple]:
    ...
    cur = conn.cursor()
    cur.execute(f"""
        SELECT conv_id, title, platform, model,
               coalesce(source_host, 'unknown')
...
def build_day_transcript(conn, convs: list[tuple]) -> str:
    ...
    blocks = []
    total = 0
    for conv_id, title, platform, model, source_host in convs:
        ...
        lines = lines[:room]
        total += len(lines)
        # chassis を header に出す。cross-family な一日 (codex/kimi/grok 混在)
        # を日記が識別できるようにするため。転記中の [CLAUDE] ラベルは chassis
        # 非依存なので、ここでしか agent の器の切替は伝わらない。
        provenance = "/".join(x for x in (platform, model, source_host) if x)
        tag = f" [{provenance}]" if provenance else ""
        header = (f"## 会話: {title}{tag}" if title else f"## 会話{tag}")
...
def _writer_provenance() -> dict[str, str]:
    return {
        "host": os.environ.get("HIPPOCAMPUS_WRITER_HOST") or socket.gethostname(),
        "runtime": os.environ.get("HIPPOCAMPUS_WRITER_RUNTIME", "hippocampus-diary"),
        "memory_mode": "generated_from_transcripts",
    }


def source_provenance(convs: list[tuple]) -> list[dict]:
    """Stable JSON-ready lineage for every session used by the writer."""
    return [
        {"session_id": conv_id, "runtime": platform, "model": model,
         "host": source_host or "unknown"}
        for conv_id, _title, platform, model, source_host in convs
    ]


def write_diary(client, date_str: str, transcript: str,
                prior: list[tuple], sources: list[dict],
                writer: dict[str, str]) -> tuple[str, str]:
    ...
    prompt = DIARY_PROMPT.format(
        ...
        writer_context=json.dumps(writer, ensure_ascii=False, sort_keys=True),
        source_context=json.dumps(sources, ensure_ascii=False, sort_keys=True),
    )
    refused = False
...
def _has_source_conv_ids(cur) -> bool:
    """True if migration 029 (diary.source_conv_ids) is applied. Cached per
    process so pre-029 databases keep working with the legacy INSERT."""
    global _SOURCE_CONV_IDS_COL
    if _SOURCE_CONV_IDS_COL is None:
        cur.execute("""
            SELECT 1 FROM information_schema.columns
            WHERE table_schema = 'personal' AND table_name = 'diary'
              AND column_name = 'source_conv_ids'
        """)
        _SOURCE_CONV_IDS_COL = cur.fetchone() is not None
    return _SOURCE_CONV_IDS_COL


_SOURCE_CONV_IDS_COL: bool | None = None


def store_diary(cur, conn, date_str: str, body: str, conv_ids: list[str],
                model_used: str, sources: list[dict], writer: dict[str, str]) -> None:
    vec = get_default_client().encode_batch(
        [body], where="diary", max_length=EMBED_MAX_LENGTH
    )[0]
    if _has_provenance_columns(cur):
        cur.execute("""
            INSERT INTO personal.diary
                (entry_date, body, dense, conv_count, model_used, source_conv_ids,
                 writer_host, writer_runtime, memory_mode, source_provenance)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s::jsonb)
            ON CONFLICT (entry_date) DO UPDATE
              SET body = EXCLUDED.body, dense = EXCLUDED.dense,
                  conv_count = EXCLUDED.conv_count,
                  model_used = EXCLUDED.model_used,
                  source_conv_ids = EXCLUDED.source_conv_ids,
                  writer_host = EXCLUDED.writer_host,
                  writer_runtime = EXCLUDED.writer_runtime,
                  memory_mode = EXCLUDED.memory_mode,
                  source_provenance = EXCLUDED.source_provenance,
                  created_at = now()
        """, (date_str, body, vec, len(conv_ids), model_used, conv_ids,
              writer["host"], writer["runtime"], writer["memory_mode"],
              json.dumps(sources, ensure_ascii=False)))
    elif _has_source_conv_ids(cur):
        # emit-from-generator (gh #64): persist the exact population this entry
        # summarized so the grounding auditor inherits it instead of re-deriving.
        cur.execute("""
            INSERT INTO personal.diary
                (entry_date, body, dense, conv_count, model_used, source_conv_ids)
            VALUES (%s, %s, %s, %s, %s, %s)
        ...
        cur.execute("""
            ...
            ON CONFLICT (entry_date) DO UPDATE
              SET body = EXCLUDED.body,
                  ...
                  model_used = EXCLUDED.model_used,
                  created_at = now()
        """, (date_str, body, vec, len(conv_ids), model_used))
    conn.commit()


def _has_provenance_columns(cur) -> bool:
    global _PROVENANCE_COLS
    if _PROVENANCE_COLS is None:
        cur.execute("""
            SELECT 1 FROM information_schema.columns
            WHERE table_schema = 'personal' AND table_name = 'diary'
              AND column_name = 'source_provenance'
        """)
        _PROVENANCE_COLS = cur.fetchone() is not None
    return _PROVENANCE_COLS


_PROVENANCE_COLS: bool | None = None
...
def process_day(client, conn, date_str: str, platforms: tuple,
    ...
    if dry_run:
        ...
        return "dry"
    sources = source_provenance(convs)
    ...
    store_diary(conn.cursor(), conn, date_str, body,
                [cid for cid, *_ in convs], model_used, sources, writer)
