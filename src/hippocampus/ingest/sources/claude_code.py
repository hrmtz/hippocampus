"""claude-code adapter — ported from scripts/ingest_new_sessions.py.
...
from ...parsers.claude_session_parser import _parse_session, extract_cwd_from_jsonl
from ..base import EmbedParams, IngestContext, SourceItem

GIT_TIMEOUT = 1.5  # parser-correctness magi recommendation (local .git < 50ms)
...
class ClaudeCodeAdapter:
    ...
    def discover(self, ctx: IngestContext) -> Iterable[SourceItem]:
        ...
        cur.execute("""
            ...
            FROM personal.conversations WHERE platform='claude_code'
        """)
    ...
    def should_ingest(self, conv: dict, ctx: IngestContext) -> bool:
        return True  # discover() already diffed; re-yields are intentional

    # ── helpers (verbatim semantics from the legacy script) ──────────────
    def _load_excluded_prefixes(self, cur) -> list[str]:
        if self._excluded_prefixes is None:
            try:
                ...
                self._excluded_prefixes = []   # pre-migration-014 fallback
