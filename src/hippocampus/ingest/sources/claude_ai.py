"""claude-ai adapter — ported from scripts/ingest_claude_ai.py.

Incremental contract: one-shot ZIP set-diff (same as chatgpt).
"""
...
from ...parsers.claude_ai_parser import parse_zip
...
class ClaudeAiAdapter:
    ...
    def discover(self, ctx: IngestContext) -> Iterable[SourceItem]:
        if not ctx.args:
            raise SystemExit("usage: hippocampus ingest claude-ai <data-export.zip>")
