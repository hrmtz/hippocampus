...
from ...parsers.antigravity_session_parser import _parse_session, extract_cwd_from_jsonl
...
class AntigravityAdapter:
    ...
    def discover(self, ctx: IngestContext) -> Iterable[SourceItem]:
        brain_dir = Path(os.environ.get(
            "ANTIGRAVITY_BRAIN_DIR", os.path.expanduser("~/.gemini/antigravity-cli/brain")))
        cur = ctx.conn.cursor()
        cur.execute("""
            SELECT conv_id, coalesce(last_file_size, 0), coalesce(msg_count, 0)
            FROM personal.conversations WHERE platform='antigravity'
        """)
        ...
        # Find transcript.jsonl under ~/.gemini/antigravity-cli/brain/*/
        for p in sorted(brain_dir.rglob("transcript.jsonl")):
            if "subagents" in p.parts:
                continue
            session_id = p.parts[-4] if len(p.parts) >= 4 else p.parent.parent.name
