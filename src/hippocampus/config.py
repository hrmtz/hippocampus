...
- Semantic embedding is OFF unless explicitly configured: BGE_EMBED_URL
  (HTTP backend) or EMBED_PROVIDER=bge-inprocess (local model, requires the
  `bge-local` extra). There is no silent in-process fallback (r2-codex-2).
...
class Settings:
    ...
    bge_embed_token: str
    embed_provider: str          # "" | "bge" | "bge-inprocess" (Phase 2b adds more)
    mcp_sse_token: str
    fastmcp_host: str
    fastmcp_port: int
    calling_chassis: str
    ...
    def embed_configured(self) -> bool:
        ...
        return bool(self.bge_embed_url) or self.embed_provider == "bge-inprocess"
    ...
    def load(cls, *, require_pg: bool = True) -> "Settings":
        ...
        if provider not in ("", "bge", "bge-inprocess"):
            raise ConfigError(
                f"EMBED_PROVIDER={provider!r} is not supported "
                "(expected: bge | bge-inprocess)"
            )
        return cls(
            pg_url=pg_url,
