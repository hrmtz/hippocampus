...
| **bge on-demand** (`bge-ondemand`) | One local machine; you want semantic search but do not want BGE-M3 resident all day | `hippocampus init --embed bge-ondemand`; first semantic ingest/search starts compose `bge`; it exits after `BGE_ONDEMAND_IDLE_SECONDS` | first semantic request waits for container/model startup; peak RAM is still ~6 GB while running; base MCP process stays light |
| **bge HTTP** (`bge-http`) | You want semantic search and are fine running one more container, or you have a GPU box elsewhere | `docker compose --profile bge up -d` (serves on `127.0.0.1:8086`; set `BGE_EMBED_TOKEN` in `.env`), then `BGE_EMBED_URL=http://localhost:8086` | model download (~6 GB) on first container start; ~6 GB RAM while the container is running; server process stays light |
| **bge HTTP, manual low-memory** (`bge-http`) | Single local machine, but you do not want BGE-M3 resident all day | Start before semantic work with `docker compose --profile bge up -d`; stop afterward with `docker compose stop bge` | same BGE-M3 peak RAM as `bge-http`, but you release it between ingest/search sessions |
...
To change later: edit `.env` (`EMBED_PROVIDER=bge-ondemand`,
`BGE_EMBED_URL`/`BGE_EMBED_TOKEN`, or `EMBED_PROVIDER=bge-inprocess`) and
restart the server.

On-demand notes:

- Code-level behavior, state files, endpoint resolution, and failure modes are
  documented in [docs/BGE_ONDEMAND.md](docs/BGE_ONDEMAND.md).
- `hippocampus doctor` is passive in `bge-ondemand` mode: it reports
  cold/hot/failed status without starting BGE-M3 and without calling `/embed`.
- The first semantic ingest/search runs `docker compose --profile bge up -d bge`
  under the hood, waits for authenticated `/ready`, then sends the embed
  request.
- `BGE_ONDEMAND_IDLE_SECONDS` defaults to `300`. When the server has no active
  embed request for that long, it shuts itself down and Docker does not restart
  it because init writes `BGE_RESTART_POLICY=no`.
- The default published port is `127.0.0.1:8086`. If that port is already in
  use and you did not set `BGE_ONDEMAND_PORT`/`BGE_ONDEMAND_URL`, the
  supervisor retries once on a free loopback port and records that URL in its
  state file.
- `hippocampus init --embed bge-ondemand` writes `HIPPOCAMPUS_COMPOSE_DIR`
  so later `hippocampus ingest ...` runs can start the same compose service
  even when launched from another current directory.
- On-demand lowers steady resident memory, not peak memory. If the machine
  cannot spare roughly 6 GB during semantic work, use `none` or a remote
  `bge-http` server instead.

Manual low-memory notes:

- `BGE_EMBED_URL=http://localhost:8086` can stay in `.env` while the local
  `bge` container is stopped. Semantic ingest/search will fail loudly until
  you start it again; this is expected if you intentionally stopped BGE-M3 to
  save RAM.
- Use `docker compose --profile bge up -d` before semantic ingest/search and
  `docker compose stop bge` afterward to release the container memory.
- The compose `bge` profile uses an `hf_cache` volume mounted as `/hf_cache`.
  If the first model download is interrupted and later starts fail during
  model load, stop `bge` and retry. If the cache is corrupt, remove only the
  compose `hf_cache` volume so BGE-M3 re-downloads; do not remove `pg_data`,
  which contains PostgreSQL data.
...
| `✗ embed: ... HTTP 404` / `unreachable` | wrong `BGE_EMBED_URL`, or the server isn't running | for manual low-memory, start it with `docker compose --profile bge up -d`; confirm the URL has no path suffix (the client appends `/embed`) |
| `– embed: bge-ondemand cold` | expected low-memory state | run a semantic ingest/search to cold-start BGE, or use `docker compose --profile bge up -d bge` to warm it manually |
| `bge-ondemand startup timed out` | first model download is still running, Docker failed, or BGE could not bind a loopback port | check `docker compose logs bge`; retry after the first download completes; if the HuggingFace cache is corrupt, remove only the compose `hf_cache` volume, not `pg_data` |
